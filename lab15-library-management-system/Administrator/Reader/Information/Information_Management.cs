...
WHERE
    reader_information.`Cid` = reader_category.`Cid`;
             */
            string query = "SELECT Rid, PASSWORD, reader_category.name AS category, reader_information.name, gender, number, registration, valid, currently, cumulative, loss, violations, remark FROM reader_information, reader_category WHERE reader_information.`Cid` = reader_category.`Cid`";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                // 日期格式为yyyy/MM/dd
                myitem.SubItems.Add(DateTime.Parse(dr["registration"].ToString()).ToString("yyyy/MM/dd"));
