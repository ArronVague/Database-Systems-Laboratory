...
- 由于图书基本信息的 ISBN 是手动输入的，因此在添加图书基本信息时，需要查重（未完成）

##### 业务管理

###### 图书借阅/归还管理 [Almost done]
...
- 功能
  ...
  - 删除
- 补充说明
  - 其实应该用更符合人性化的方式做借阅和归还，但是由于时间原因，暂时不做
  - 而且借阅前没有查询图书是否可借
  - 需要用触发器实现图书状态更新
    - insert 触发馆藏图书信息表中图书状态的更新
    - update 需要判断归还时间是否为空，若为空不触发，若不为空则触发
...
- 特殊地，系统登陆也是完整模块
- 数据可能包含单引号，因此需要手动转义，例如 `O'Reilly` 需要写成 `O''Reilly`或`O\\'Reilly`
